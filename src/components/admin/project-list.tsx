...
import { togglePublished, deleteProject, reorderProjects } from '@/app/actions/projects'
...
export function ProjectList({ projects }: { projects: Project[] }) {
  ...
  function handleDragEnd(result: DropResult) {
    ...
    const reordered = Array.from(optimisticProjects)
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="hidden md:grid grid-cols-[24px_64px_1fr_160px_80px_120px_160px] gap-4 px-4 py-2 text-xs uppercase tracking-wider text-text-muted border-b border-white/10">
        <span>Reorder</span>
        <span>Thumb</span>
                  ...
                  <Draggable key={project.id} draggableId={project.id} index={index}>
                    {(provided) => (
                      <div
                        ...
                        className="bg-bg-card border border-white/10 rounded-lg px-4 py-3 flex flex-col md:grid md:grid-cols-[24px_64px_1fr_160px_80px_120px_160px] md:items-center gap-3 md:gap-4"
                      >
                        {/* Drag handle */}
                        <div
                          {...provided.dragHandleProps}
                          className="cursor-grab text-text-muted hover:text-text-primary select-none text-sm leading-none hidden md:flex items-center justify-center"
